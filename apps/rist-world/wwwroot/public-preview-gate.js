(()=>{
 ...
 function handoffCover(){
  if(document.querySelector('.rist-auth-handoff-cover'))return;
  const cover=document.createElement('div');
  cover.className='rist-auth-handoff-cover';
  cover.setAttribute('aria-hidden','true');
  cover.style.cssText='position:fixed;inset:0;z-index:2147483646;background:#03050a url("assets/branding/shaelvien-dragon-creation-startup-screen.png") center/cover no-repeat;pointer-events:none';
  document.body.append(cover);
 }
 function clearHandoffCover(){document.querySelector('.rist-auth-handoff-cover')?.remove()}
 function openStart(options){
  window.RistGameStartScreen?.open?.(options);
  requestAnimationFrame(clearHandoffCover);
 }
 ...
 function gate(){
  ...
  wrap.addEventListener('click',e=>{const b=e.target.closest?.('[data-entry]');if(!b)return;const action=b.dataset.entry;if(action==='preview'){lockPreview();wrap.remove();openStart({preview:true})}else auth(action)});
 ...
 function start(){
  ...
  if(preview){lockPreview();openStart({preview:true})}
  else if(sessionStorage.getItem('rist.session'))openStart({preview:false});
  else if(params.has('rist_handoff')||location.hash.includes('rist_session')){
   handoffCover();
   let attempts=0;const timer=setInterval(()=>{if(sessionStorage.getItem('rist.session')){clearInterval(timer);openStart({preview:false})}else if(++attempts>=100){clearInterval(timer);gate()}},100);
  }
  else gate();
  observe();
 }
