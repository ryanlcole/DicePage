(()=>{
  ...
  function apply(map,x,y){
    if(!map)return;
    x=clamp(x,-18,18);y=clamp(y,-14,14);
  ...
  function pointerDepth(event){
    ...
    const map=activeMouseMap||mapFor(event.target);if(!map)return;
    const rect=map.getBoundingClientRect();if(rect.width<=0||rect.height<=0)return;
  ...
  document.addEventListener('pointerdown',event=>{
    const map=mapFor(event.target);if(!map)return;
    if(!event.pointerType||event.pointerType==='mouse'){
      if(event.button===0)activeMouseMap=map;
    }else{
      void window.RistRuntime?.requestOrientation?.();
    }
  },{passive:true,capture:true});
  document.addEventListener('pointermove',pointerDepth,{passive:true,capture:true});
  document.addEventListener('pointerup',event=>{if(!event.pointerType||event.pointerType==='mouse')activeMouseMap=null},{passive:true,capture:true});
  document.addEventListener('pointercancel',()=>{activeMouseMap=null},{passive:true,capture:true});
  addEventListener('rist:orientation',event=>{
    const map=document.querySelector('.map');if(!map)return;
    const gamma=Number(event.detail?.gamma)||0,beta=Number(event.detail?.beta)||0;
    apply(map,clamp(gamma/3,-14,14),clamp((beta-45)/5,-10,10));
  },{passive:true});

  window.RistFauxDepth={apply,requestMotion:()=>window.RistRuntime?.requestOrientation?.()};
})();
