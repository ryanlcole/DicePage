(()=>{
 ...
 function removeSumControls(root){
  qa('.rist-dice-sum-result,.native-sum-toggle,.sum',root).forEach(el=>el.remove());
  qa('button,output',root).forEach(el=>{if(/^SUM\s*-?\d*$/i.test((el.textContent||'').trim())||/^SUM$/i.test((el.textContent||'').trim()))el.remove()});
 }
