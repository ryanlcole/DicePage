(()=>{
 ...
 function clearLandingParallax(root){
  if(!root)return;
  for(const selector of ['.launcher-hero','.launcher-primary','.launcher-secondary','.launcher-footer']){const node=root.querySelector(selector);if(node){node.style.translate='';node.style.willChange=''}}
 }
 function applyLandingParallax(){
  const root=landing();if(!root)return;
  if(!userParallaxEnabled()){clearLandingParallax(root);return}
  const hero=root.querySelector('.launcher-hero'),primary=root.querySelector('.launcher-primary'),secondary=root.querySelector('.launcher-secondary'),footer=root.querySelector('.launcher-footer');
 ...
 function clearWorldBuilderParallax(){
  ...
  root.querySelectorAll('.world-stage .tile-cell').forEach(tile=>{tile.style.setProperty('--wb-motion-x','0px');tile.style.setProperty('--wb-motion-y','0px');tile.style.willChange='';tile.dataset.parallaxMotion='0'})
 }
 function applyWorldBuilderParallax(){
  ...
  list.forEach((tile,index)=>{
   ...
   tile.style.setProperty('--wb-motion-y',`${dy.toFixed(2)}px`);
   tile.style.willChange=motion>0?'transform':'';
   tile.dataset.parallaxMotion=motion.toFixed(3)
  })
 }

 function apply(){
  state.raf=0;state.x+=(state.targetX-state.x)*.16;state.y+=(state.targetY-state.y)*.16;
  applyLandingParallax();applyWorldBuilderParallax();
  if(Math.abs(state.targetX-state.x)>.05||Math.abs(state.targetY-state.y)>.05)state.raf=requestAnimationFrame(apply)
 }
 ...
 function onPointerMove(event){
  const canvas=event.target?.closest?.('.worldbuilder-studio .studio-viewer-canvas');if(!canvas||!worldBuilderParallaxActive()||!userParallaxEnabled())return;
  const rect=canvas.getBoundingClientRect();if(rect.width<1||rect.height<1)return;
  state.targetX=clamp(((event.clientX-rect.left)/rect.width-.5)*18,-9,9);state.targetY=clamp(((event.clientY-rect.top)/rect.height-.5)*14,-7,7);schedule()
 }
