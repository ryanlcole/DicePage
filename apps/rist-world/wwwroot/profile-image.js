(()=>{
 ...
 function imageFor(name){
  ...
  const alias=currentAlias();if(!name||!alias||name===alias)return localStorage.getItem(IMAGE_KEY)||'';
  return '';
 }
 function persistImage(data){
  ...
  document.dispatchEvent(new CustomEvent('rist:profile-image-changed',{detail:{alias,image:data||''}}));
 }
 function cropData(img,zoom,x,y){
  ...
  const px=(Number(x)||50)/100,py=(Number(y)||50)/100;
  const dx=(size-w)/2+((px-.5)*size),dy=(size-h)/2+((py-.5)*size);
  ctx.clearRect(0,0,size,size);ctx.drawImage(img,dx,dy,w,h);return canvas.toDataURL('image/png',.92);
 }
 function editorHtml(){return `<div class="rist-profile-image-editor"><div class="rist-profile-preview"><img alt="Profile preview"></div><label class="rist-profile-file">Choose image<input type="file" accept="image/*" data-profile-file></label><label>Zoom <input type="range" min="1" max="3" step="0.01" value="1" data-profile-zoom></label><label>Horizontal <input type="range" min="0" max="100" value="50" data-profile-x></label><label>Vertical <input type="range" min="0" max="100" value="50" data-profile-y></label><div class="rist-profile-actions"><button type="button" data-profile-save>Save image</button><button type="button" data-profile-clear>Clear</button></div></div>`}
 ...
 function wireEditor(host){
  if(host.dataset.profileWired==='1')return;host.dataset.profileWired='1';const img=q('.rist-profile-preview img',host),file=q('[data-profile-file]',host);sourceData=localStorage.getItem(IMAGE_KEY)||'';if(sourceData)img.src=sourceData;
 ...
 function patchSettings(){
  ...
  const avatar=q('.rist-account-avatar',profile),saved=localStorage.getItem(IMAGE_KEY)||'';if(avatar){avatar.innerHTML=saved?`<img src="${saved}" alt="">`:'';}
 }
 function selectedPrivateName(){return q('.rist-private-user-select')?.value||''}
 function isPrivate(){const active=qa('.rist-chat-themed-tabs button').find(b=>b.classList.contains('active'));return (active?.textContent||'').trim().toLowerCase()==='private'}
 function patchPrivatePortrait(){
  if(!isPrivate())return;const name=selectedPrivateName(),src=imageFor(name);const output=q('.rist-themed-chat-output');if(!output)return;
  let portrait=q('.rist-themed-chat-portrait',output);if(!portrait){portrait=document.createElement('div');portrait.className='dialogue-portrait rist-themed-chat-portrait';output.prepend(portrait)}
  portrait.replaceChildren();if(src){const img=document.createElement('img');img.src=src;img.alt=name?`${name} profile`:'';portrait.appendChild(img)}else{const back=document.createElement('span');back.className='dialogue-card-back';back.setAttribute('aria-hidden','true');portrait.appendChild(back)}
  const speaker=q('.rist-themed-chat-copy strong',output);if(speaker&&name)speaker.textContent=name;
 }
 function scan(){patchSettings();patchPrivatePortrait()}
 let queued=false;function queue(){if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;scan()})}
 document.addEventListener('rist:private-recipient-changed',patchPrivatePortrait);document.addEventListener('rist:profile-image-changed',patchPrivatePortrait);document.addEventListener('click',e=>{if(e.target.closest?.('.rist-chat-themed-tabs'))setTimeout(patchPrivatePortrait,0)});
 ...
 window.RistProfileImage={imageFor,current:()=>localStorage.getItem(IMAGE_KEY)||''};
})();
