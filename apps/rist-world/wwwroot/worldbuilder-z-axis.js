...
export function attach(element,dotnet){
 ...
 .worldbuilder-studio.viewer-locked .desktop-map-zoom{opacity:.45!important}`;
 ...
 async function syncUi(){
  ...
  try{
   ...
   let custom=r.querySelector('[data-wb-command="custom-size"]');if(!custom&&size){custom=document.createElement('button');custom.type='button';custom.className='wb-injected-command';custom.dataset.wbCommand='custom-size';custom.innerHTML=`<strong>Custom</strong><small>${customMapScale} / square</small>`;custom.onclick=openCustomSize;size.insertAdjacentElement('afterend',custom);}else if(custom){const sm=custom.querySelector('small');if(sm)sm.textContent=`${customMapScale} / square`;}
   const lock=byText('Z-Lock')||byText('Lock')||byText('Unlock');if(lock){wire(lock,async()=>{try{viewerLocked=await dotnet.invokeMethodAsync('ToggleViewerLockFromJs');localStorage.setItem('rist.world.viewerLocked',String(viewerLocked));}catch{viewerLocked=!viewerLocked;}scheduleUi();});const s=lock.querySelector('strong'),sm=lock.querySelector('small'),label=viewerLocked?'Unlock':'Lock',sub=viewerLocked?'Map + Viewer Locked':'Map + Viewer Unlocked';if(s&&s.textContent!==label)s.textContent=label;if(sm&&sm.textContent!==sub)sm.textContent=sub;lock.classList.toggle('active',viewerLocked);}
