import * as core from './worldbuilder-z-axis-core.js?v=20260914-unified-gesture-1';
...
export function attach(element,dotnet){
 ...
 const blocked=t=>!!t?.closest?.('.studio-mini-panel,.studio-load-panel,.studio-library-shade,.recursion-cockpit,.locked-tile-menu,.recursive-region-actions,.wb-modal,.asset-preview-stage');
 ...
 const openPublish=()=>modal('Publish',(host,close)=>host.append(button('Publish',async()=>{try{await dotnet.invokeMethodAsync('SetPublishModeFromJs',true)}catch{}close()}),button('Unpublish',async()=>{try{await dotnet.invokeMethodAsync('SetPublishModeFromJs',false)}catch{}close()})));
 ...
 const updateDepth=async()=>{try{depthState=await dotnet.invokeMethodAsync('GetWorldBuilderDepthState');viewerLocked=!!(depthState.viewerLocked??depthState.ViewerLocked)}catch{}studio?.classList.toggle('viewer-locked',viewerLocked);studio?.classList.toggle('viewer-unlocked',!viewerLocked)};
 async function syncUi(){
  ...
  try{
   ...
   await updateDepth();
   const lock=byText('Z-Lock')||byText('Lock')||byText('Unlock');if(lock){wire(lock,async()=>{try{viewerLocked=await dotnet.invokeMethodAsync('ToggleViewerLockFromJs');localStorage.setItem('rist.world.viewerLocked',String(viewerLocked));window.ristViewerNavigation?.resync?.()}catch{viewerLocked=!viewerLocked}scheduleUi()});const strong=lock.querySelector('strong'),small=lock.querySelector('small');if(strong)strong.textContent=viewerLocked?'Unlock':'Lock';if(small)small.textContent=viewerLocked?'Viewer Locked':'Viewer Unlocked';lock.classList.toggle('active',viewerLocked);lock.setAttribute('aria-pressed',viewerLocked?'true':'false')}
 ...
 return{dispose(){disposed=true;active=null;window.ristMovePlacement?.cancel?.();clearInterval(autoSaveTimer);uiObserver?.disconnect();document.removeEventListener('pointerdown',down,true);document.removeEventListener('pointermove',move,true);document.removeEventListener('pointerup',up,true);document.removeEventListener('pointercancel',cancel,true);document.removeEventListener('contextmenu',context,true);document.removeEventListener('dragstart',dragstart,true);style.remove();clearModal();try{coreBinding?.dispose?.()}catch{}}};
}
