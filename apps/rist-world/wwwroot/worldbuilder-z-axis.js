...
export function attach(element,dotnet){
 ...
 async function syncUi(){
  ...
  try{
   ...
   if(size){wire(size,cycleTileSize);const s=size.querySelector('strong');if(s)s.innerHTML=`${tileSizeKm}<sup>2</sup>`;let small=size.querySelector('small');if(!small){small=document.createElement('small');size.append(small);}if(small.textContent!=='Tile Size')small.textContent='Tile Size';}
