...
export function attach(element,dotnet){
 ...
 let distancePerSquareKm=Number(localStorage.getItem('rist.world.distancePerSquareKmAtZ0')||localStorage.getItem('rist.world.tileSizeKmAtOrigin'))||1;
 ...
 const setDistancePerSquare=async km=>{if(!Number.isFinite(km)||km<=0)return;distancePerSquareKm=Math.min(1_000_000,Math.max(.001,km));localStorage.setItem('rist.world.distancePerSquareKmAtZ0',String(distancePerSquareKm));try{await dotnet.invokeMethodAsync('SetDistancePerSquareKmAtZ0FromJs',distancePerSquareKm);}catch{}scheduleUi();};

 const openCustomSize=()=>modal('Distance per square at Z=0',(host,close)=>{const label=document.createElement('label');label.append(document.createTextNode('Kilometers per viewer square'));const input=document.createElement('input');input.type='number';input.min='0.001';input.max='1000000';input.step='any';input.value=String(distancePerSquareKm);label.append(input);host.append(label);const note=document.createElement('small');note.textContent='Defines true world scale at Z=0. Tile Size remains the number of grid squares occupied by the asset.';host.append(note);host.append(button('Apply',async()=>{await setDistancePerSquare(Number(input.value));close();}),button('Cancel',close));});
 ...
 async function syncUi(){
  ...
  try{
   ...
   if(size){let small=size.querySelector('small');if(!small){small=document.createElement('small');size.append(small);}if(small.textContent!=='Tile Size')small.textContent='Tile Size';}
   let custom=r.querySelector('[data-wb-command="custom-size"]');if(!custom&&size){custom=document.createElement('button');custom.type='button';custom.className='wb-injected-command';custom.dataset.wbCommand='custom-size';custom.innerHTML='<strong>Square Scale</strong><small>Z=0 distance</small>';custom.onclick=openCustomSize;size.insertAdjacentElement('afterend',custom);}
 ...
 try{viewerLocked=localStorage.getItem('rist.world.viewerLocked')!=='false';dotnet.invokeMethodAsync('SetViewerLockFromJs',viewerLocked).catch(()=>{});dotnet.invokeMethodAsync('SetDistancePerSquareKmAtZ0FromJs',distancePerSquareKm).catch(()=>{});}catch{}
