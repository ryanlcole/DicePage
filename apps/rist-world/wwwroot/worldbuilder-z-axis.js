...
export function attach(element,dotnet){
 ...
 style.textContent=`@import url('./css/worldbuilder-control-rework.css?v=20260910-controls-2');
 ...
 const openCustomSize=async()=>{let value=1;try{value=Number(await dotnet.invokeMethodAsync('GetTileSizeKmAtOriginFromJs'))||1;}catch{}modal('Tile Size at (0,0)',(host,close)=>{const label=document.createElement('label');label.append(document.createTextNode('Kilometers per tile'));const input=document.createElement('input');input.type='number';input.min='0.001';input.max='1000000';input.step='any';input.value=String(value);label.append(input);host.append(label);const note=document.createElement('small');note.textContent='World math only; travel distance remains a game-system decision.';host.append(note);host.append(button('Apply',async()=>{const km=Number(input.value);if(Number.isFinite(km)&&km>0){try{await dotnet.invokeMethodAsync('SetTileSizeKmAtOriginFromJs',km);localStorage.setItem('rist.world.tileSizeKmAtOrigin',String(km));}catch{}close();scheduleUi();}}),button('Cancel',close));});};
 ...
 async function syncUi(){
  ...
  try{
   ...
   if(size){const preset=Number(size.dataset.footprint||1);try{await dotnet.invokeMethodAsync('SetTileSizeKmAtOriginFromJs',preset);}catch{}const s=size.querySelector('strong');if(s&&s.textContent!=='Tile Size')s.textContent='Tile Size';let small=size.querySelector('small');if(!small){small=document.createElement('small');size.append(small);}const txt=`${preset} km/tile`;if(small.textContent!==txt)small.textContent=txt;}
   ...
   const view=byText('View'),undo=r.querySelector('[data-wb-command="undo"]'),remove=r.querySelector('[data-wb-command="remove"]');if(view){if(undo&&undo.nextElementSibling!==remove)r.insertBefore(undo,view);if(remove&&remove.nextElementSibling!==view)r.insertBefore(remove,view);}
 ...
 try{viewerLocked=localStorage.getItem('rist.world.viewerLocked')!=='false';dotnet.invokeMethodAsync('SetViewerLockFromJs',viewerLocked).catch(()=>{});const saved=Number(localStorage.getItem('rist.world.tileSizeKmAtOrigin'));if(Number.isFinite(saved)&&saved>0)dotnet.invokeMethodAsync('SetTileSizeKmAtOriginFromJs',saved).catch(()=>{});}catch{}
