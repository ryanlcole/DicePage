(()=>{
 ...
 const complete=name=>{
  ...
  state.phase=values[name]??state.phase;
  state.phaseName=name;
  document.documentElement.dataset.ristPhase=name;
  announce('complete',name);
 };

 function loadScript(src){
  const key=canonical(src);
  if(state.loaded.has(key))return state.loaded.get(key);
  const existing=[...document.scripts].find(s=>{try{return canonical(s.src)===key}catch{return false}});
  if(existing?.dataset.ristLoaded==='1'){
   const ready=Promise.resolve(existing);
   state.loaded.set(key,ready);
   return ready;
  }
  const task=new Promise(resolve=>{
   const script=existing||document.createElement('script');
   const finish=ok=>{
    script.dataset.ristLoaded='1';
    if(!ok){
     const error={src,key,phase:state.phaseName,time:new Date().toISOString()};
     state.errors.push(error);
     console.error('[RIST PLC] module failed',src);
     dispatchEvent(new CustomEvent('rist:module-error',{detail:error}));
    }
    resolve(script);
   };
   if(existing){
   ...
   script.src=src;
   script.async=false;
   script.dataset.ristPhaseLoad='1';
   script.addEventListener('load',()=>finish(true),{once:true});
   script.addEventListener('error',()=>finish(false),{once:true});
   document.body.appendChild(script);
  });
  state.loaded.set(key,task);
  return task;
 }

 async function ensureGroup(name){
  if(state.completed.has(name))return;
  if(state.running.has(name))return state.running.get(name);
  const task=(async()=>{
   const dependency=dependencies[name];
   if(dependency)await ensureGroup(dependency);
   state.started.add(name);
   announce('start',name);
   for(const src of groups[name]||[])await loadScript(src);
   complete(name);
  })();
  state.running.set(name,task);
  try{await task}finally{state.running.delete(name)}
 }
 ...
 async function scanInteraction(){
  await ensureGroup('interaction');
  state.phase=PHASE.COMPLETE;
  state.phaseName='complete';
  document.documentElement.dataset.ristPhase='complete';
  dispatchEvent(new CustomEvent('rist:runtime-phase',{detail:{kind:'complete',name:'complete',value:PHASE.COMPLETE,completed:[...state.completed]}}));
 }

 /* One scheduler shared by DOM-patching modules. */
 const scheduled=new Map();
 function frame(key,callback){
  if(scheduled.has(key))return;
  scheduled.set(key,requestAnimationFrame(()=>{
   scheduled.delete(key);
   try{callback()}catch(error){console.error('[RIST runtime]',key,error)}
  }));
 }
 ...
 addEventListener('resize',signalViewport,{passive:true});
 addEventListener('orientationchange',signalViewport,{passive:true});
 ...
 const core=()=>void scanCore();
 const interaction=()=>void scanInteraction();
 addEventListener('rist:app-ready',core,{once:true});
 setTimeout(core,10000);
 addEventListener('pointerdown',interaction,{once:true,capture:true,passive:true});
 addEventListener('keydown',interaction,{once:true,capture:true});
 if(!matchMedia('(max-width:800px)').matches){
  const idle=window.requestIdleCallback||((fn)=>setTimeout(fn,1800));
  idle(interaction,{timeout:5000});
 }

 window.RistRuntime={frame,signalDom,signalViewport};
 window.RistPLC={PHASE,state,ensureGroup,scanCore,scanInteraction,loadScript};
})();
