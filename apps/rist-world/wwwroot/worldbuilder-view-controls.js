(()=>{
 ...
 const lockButton=()=>railButtons().find(button=>{const strong=button.querySelector('strong')?.textContent?.trim();const small=button.querySelector('small')?.textContent?.trim().toLowerCase()||'';return strong==='Z-Lock'||strong==='Lock'||strong==='Unlock'||small.includes('viewer locked')||small.includes('viewer unlocked')})||null;
