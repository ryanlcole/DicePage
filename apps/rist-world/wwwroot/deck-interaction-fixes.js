(()=>{
 ...
 function rollDie(index){const source=liveDiceSources()[index%DICE_COUNT];const button=source?.matches?.('.die-button')?source:source?.querySelector?.('.die-button');button?.click()}
 function syncD5(index,value){
  const normalized=String(Math.max(1,Math.min(5,Number(value)||1)));
  const source=liveDiceSources()[index%DICE_COUNT];const select=source?.querySelector?.('select');
  ...
  qa('.rist-dice-image-loop > .rist-dice-proxy').forEach((proxy,i)=>{
   ...
   const valueNode=proxy.querySelector('.rist-d5-step-value');if(valueNode)valueNode.textContent=`${index%DICE_COUNT===1?'+':'-'}${normalized}`;
  });
 }
 function ensureTripleLoop(loop){
  if(!loop)return;const children=[...loop.children];if(children.length<DICE_COUNT)return;
  while(loop.children.length>DICE_COUNT*3)loop.lastElementChild?.remove();
  if(loop.children.length===DICE_COUNT*2){children.slice(0,DICE_COUNT).forEach((node,index)=>{const clone=node.cloneNode(true);clone.setAttribute('aria-hidden','true');clone.dataset.loopIndex=String(index);loop.appendChild(clone)})}
 ...
 function ensureD5Stepper(proxy,index){
  if(!proxy?.classList.contains('d5-control')||proxy.querySelector('.rist-d5-stepper'))return;
  ...
  const value=document.createElement('span');value.className='rist-d5-step-value';value.textContent=`${index===1?'+':'-'}${select.value||1}`;
  const plus=document.createElement('button');plus.type='button';plus.className='rist-d5-step-plus';plus.textContent='+';
  const adjust=delta=>{const current=Number(select.value)||1;syncD5(index,Math.max(1,Math.min(5,current+delta)))};
 ...
 function patchDice(){
  ...
  ensureTripleLoop(head);ensureTripleLoop(art);head.classList.remove('rist-dice-synced');art.classList.remove('rist-dice-synced');
  qa('.rist-dice-proxy',art).forEach((proxy,i)=>{const idx=i%DICE_COUNT;if(idx===1||idx===2)ensureD5Stepper(proxy,idx)});
  if(row1.dataset.diceLinked!=='1'){
   ...
   row2.addEventListener('click',event=>{if(event.target.closest('.rist-d5-stepper,select'))return;const proxy=event.target.closest('.rist-dice-image-loop > .rist-dice-proxy');if(!proxy)return;event.preventDefault();event.stopImmediatePropagation();rollDie(Number(proxy.dataset.loopIndex||0))},true);
