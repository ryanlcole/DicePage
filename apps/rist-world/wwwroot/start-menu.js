(()=>{
 ...
 async function hydrateAccount(){const out=panel?.querySelector('.rist-discord-id');if(!out)return;try{const cfg=await fetch('auth-config.json',{cache:'no-store'}).then(r=>r.json());const token=sessionStorage.getItem('rist.session');if(!cfg?.apiBaseUrl||!token){out.textContent='Log in with Discord';return}const r=await fetch(cfg.apiBaseUrl.replace(/\/$/,'')+'/me',{headers:{Authorization:'Bearer '+token},cache:'no-store'});if(!r.ok){out.textContent='Unavailable';return}const p=await r.json();out.textContent=p.userId||p.UserId||'Unavailable';}catch{out.textContent='Unavailable'}}
 function showSection(panel,name){panel.innerHTML=`<section class="rist-start-sub">${sections[name]}<button type="button" class="rist-start-back" data-start-back>Back</button><button type="button" class="rist-start-exit" data-start-exit>Exit</button></section>`;if(name==='Account')hydrateAccount()}
