(()=>{
 ...
 const sections={
  ...
  Dice:`<h2>Dice</h2><p class="rist-start-note">GameMaster table controls. Disabling SUM leaves the dice visible but requires players to add their own results.</p>${row('Show SUM','<button type="button" data-dice-sum-toggle>Loading…</button>')}<p class="rist-start-note" data-dice-gm-note></p>`,
 ...
 function showSection(name){panel.innerHTML=`<section class="rist-start-sub">${sections[name]}<button type="button" class="rist-start-back" data-start-back>Back</button><button type="button" class="rist-start-exit" data-start-exit>Exit</button></section>`;if(name==='Account')void hydrateAccount();if(name==='Dice')hydrateDice()}
 function ensure(){
