(()=>{
 ...
 function ensureStyle(){
  ...
  link.id=STYLE_ID;
  link.rel='stylesheet';
  link.href='./css/worldbuilder-device-shell.css?v=20260915-device-keyboard-widgets-1';
 ...
 function renderLabelEditor(){
  ...
  const state=labelDraft?._state;[
   descriptor('Bold',state?.bold?'On':'Off',()=>styleLabel({bold:!labelDraft._state.bold}),{active:!!state?.bold,disabled:!state}),descriptor('Italic',state?.italic?'On':'Off',()=>styleLabel({italic:!labelDraft._state.italic}),{active:!!state?.italic,disabled:!state}),descriptor('Size −',state?`${state.size}px`:'—',()=>styleLabel({size:Math.max(8,labelDraft._state.size-2)}),{disabled:!state}),descriptor('Size +',state?`${state.size}px`:'—',()=>styleLabel({size:Math.min(96,labelDraft._state.size+2)}),{disabled:!state}),descriptor('Outline',state?.outline?'On':'Off',()=>styleLabel({outline:!labelDraft._state.outline}),{active:!!state?.outline,disabled:!state}),descriptor('←','Move label',()=>moveLabel(-8,0),{disabled:!state}),descriptor('↑','Move label',()=>moveLabel(0,-8),{disabled:!state}),descriptor('↓','Move label',()=>moveLabel(0,8),{disabled:!state}),descriptor('→','Move label',()=>moveLabel(8,0),{disabled:!state}),descriptor('Commit','World writer',commitLabel,{disabled:!state,wide:true}),descriptor('Remove','Draft',removeLabelDraft,{disabled:!state,danger:true})
