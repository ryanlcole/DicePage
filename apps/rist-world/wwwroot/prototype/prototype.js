...
const MAP_TRUTH=Object.freeze({
  id:'geonaph',origin:Object.freeze({x:0,y:0,z:0}),coordinateAuthority:'XYZ',layersPerTier:LAYERS_PER_TIER,
...
let naturalWidth=1,naturalHeight=1,scale=1,minScale=.1,maxScale=12,x=0,y=0,fitX=0,fitY=0,panStart=null,pinchStart=null,viewerZ=0,activeStratum='surface',parallaxOverride=false,focusPath=[],focusSelected='surface',keyboardMode='Viewer',toolMode='Inspect',lastMix={zoomZ:0,highlands:0,mountains:0};
...
function rootByKey(key){return ROOT_OPTIONS.find(s=>s.key===key)||ROOT_OPTIONS.find(s=>s.key==='surface')}
...
function currentCategory(){const locked=focusPath.find(p=>p.level==='Category');if(locked)return locked.key;if(focusPath.length===1)return selectedOption()?.key||'';return''}
...
function parallaxMix(){
  ...
  let highlandAlpha=parallaxOverride?.92:smoothstep(2.2,5.8,zoomZ)*.92;
  let mountainAlpha=parallaxOverride?.9:smoothstep(5.2,8.8,zoomZ)*.9;
  if(category==='planes'){highlandAlpha*=.68;mountainAlpha*=.45}
  else if(category==='hills'){highlandAlpha=Math.min(1,highlandAlpha*1.08);mountainAlpha*=.72}
  else if(category==='mountain'||category==='volcano'){highlandAlpha=Math.min(1,highlandAlpha*.92);mountainAlpha=Math.min(1,mountainAlpha*1.1)}
...
function applyParallax(mix=parallaxMix()){
  ...
  surface.style.opacity='1';surface.style.transform='translate3d(0,0,0)';
  const layers=[{node:highlands,sceneZ:4,alpha:mix.highlands},{node:mountains,sceneZ:7,alpha:mix.mountains}];
  for(const layer of layers){
    const strength=reducedMotion?0:(layer.sceneZ/LAYERS_PER_TIER)*.085;
...
function updateReadouts(){
  const s=stratumByKey(activeStratum),z=splitZ(viewerZ),mix=lastMix;
  ...
  zoomLabel.textContent=`${Math.abs(x-fitX)<.5&&Math.abs(y-fitY)<.5&&Math.abs(scale-minScale)<.0001?'Fit · ':''}${percent}% · PZ ${mix.zoomZ.toFixed(1)}`;
  stratumNote.innerHTML=`<strong>${label}</strong> · Z ${viewerZ} · H ${Math.round(mix.highlands*100)}% · M ${Math.round(mix.mountains*100)}%`;
  stratumNote.setAttribute('aria-label',`${label}. Viewer Z ${viewerZ}. Highlands parallax ${Math.round(mix.highlands*100)} percent. Mountains parallax ${Math.round(mix.mountains*100)} percent. Surface remains the canonical reference plane.`);
}
function applyTransform(){
  world.style.width=naturalWidth+'px';world.style.height=naturalHeight+'px';world.style.transform=`translate3d(${x}px,${y}px,0) scale(${scale})`;
  applyParallax();updateReadouts();
}
function fitMap(){
  if(!naturalWidth||!naturalHeight)return;
  const r=stage.getBoundingClientRect();
  minScale=Math.min(r.width/naturalWidth,r.height/naturalHeight);
  scale=minScale;maxScale=Math.max(minScale*24,8);
  x=fitX=(r.width-naturalWidth*scale)/2;y=fitY=(r.height-naturalHeight*scale)/2;
  applyTransform();
}
function zoomAt(cx,cy,factor){
  ...
  const wx=(sx-x)/old,wy=(sy-y)/old;scale=next;x=sx-wx*scale;y=sy-wy*scale;applyTransform();
}
...
function jumpStratum(key){
  if(key==='all'){
    parallaxOverride=true;activeStratum='surface';viewerZ=0;focusPath=[];focusSelected='all';shortcut.value='all';renderState();announce('All parallax viewer mode. All registered major visual layers are visible from the same XYZ truth.');return;
  }
  const s=stratumByKey(key);parallaxOverride=false;activeStratum=s.key;viewerZ=s.z;focusPath=[];focusSelected=s.key;shortcut.value=s.key;renderState();announce(`${s.label} viewer shortcut. Z ${s.z}. Focus reset; map truth unchanged.`);
}
function selectFocus(key){const match=focusOptions().find(o=>o.key===key);if(!match)return;focusSelected=match.key;if(focusPath.length===0){if(match.key==='all'){parallaxOverride=true;shortcut.value='all'}else{const s=stratumByKey(match.key);activeStratum=s.key;viewerZ=s.z;parallaxOverride=false;shortcut.value=s.key}}renderState();announce(`${match.label} selected. Lock to reveal ${focusPath.length===0?'Category':nextLevel()}.`)}
function lockFocus(){
  ...
  focusSelected=focusOptions()[0]?.key||'';renderState();announce(match.level==='Entity'?`${match.label} locked. Battle Instance revealed.`:`${match.label} locked. ${nextLevel()} revealed.`);
}
...
function renderFocus(){
  ...
  focusLevel.textContent=level;focusSelect.replaceChildren();
  if(opts.length){opts.forEach(o=>{const el=document.createElement('option');el.value=o.key;el.textContent=o.label;focusSelect.append(el)});focusSelect.value=focusSelected}
  else{const el=document.createElement('option');el.textContent=entityLocked?'Battle Instance':'No content';el.value='';focusSelect.append(el)}
  focusSelect.disabled=!opts.length||entityLocked;focusLock.disabled=!opts.length||entityLocked;focusLock.classList.toggle('active',!!opts.length&&!entityLocked);focusLock.textContent=entityLocked?'LOCKED':'LOCK';focusBack.disabled=!focusPath.length;
  ...
  battle.hidden=!entityLocked;if(entityLocked){const e=focusPath.find(p=>p.level==='Entity');battleText.textContent=`${e?.label||'Entity'} focus · tactical viewer representation. Canonical XYZ identity remains unchanged.`}
}
...
function renderKeyboardKeys(){
  if(!keyboardKeys)return;keyboardKeys.replaceChildren();const z=splitZ(viewerZ);
  ...
  if(keyboardMode==='Selected'){keyboardKeys.append(toolKey('LOCK','focus',lockFocus,!focusOptions().length||focusPath.some(p=>p.level==='Entity')),toolKey('BACK','focus',unlockFocus,!focusPath.length),toolKey('HOME','Surface',resetFocus),toolKey('INSPECT','viewer',()=>setTool('Inspect')),toolKey('META','viewer',()=>setTool('Metadata')));return}
...
MAP_TRUTH.assets.forEach(asset=>{
  ...
  node.addEventListener('load',()=>{layerReady[asset.key]=true;if(asset.key==='surface'){naturalWidth=node.naturalWidth||1;naturalHeight=node.naturalHeight||1;loading.hidden=true;fitMap()}else renderState()});
  node.addEventListener('error',()=>{layerReady[asset.key]=false;if(asset.key==='surface'){loading.hidden=false;loading.textContent='SURFACE MAP ASSET UNAVAILABLE'}renderState()});
  node.src=ASSET_ROOT+asset.file;
});
shortcut.addEventListener('change',()=>jumpStratum(shortcut.value));
focusSelect.addEventListener('change',()=>selectFocus(focusSelect.value));focusLock.addEventListener('click',lockFocus);focusBack.addEventListener('click',unlockFocus);$('focusReset').addEventListener('click',resetFocus);
$('fit').addEventListener('click',fitMap);$('zoomIn').addEventListener('click',()=>{const r=stage.getBoundingClientRect();zoomAt(r.left+r.width/2,r.top+r.height/2,1.3)});$('zoomOut').addEventListener('click',()=>{const r=stage.getBoundingClientRect();zoomAt(r.left+r.width/2,r.top+r.height/2,1/1.3)});
$('back').addEventListener('click',()=>{if(history.length>1)history.back();else location.href='/Game/index.html'});
keyboardToggle.addEventListener('click',()=>keyboard.hidden?openKeyboard():closeKeyboard());$('keyboardClose').addEventListener('click',closeKeyboard);
...
stage.addEventListener('pointerdown',e=>{
  if(e.target instanceof Element&&e.target.closest('[data-ui]'))return;if(e.pointerType==='mouse'&&e.button!==0)return;
  ...
  else if(pointers.size===2){const[a,b]=[...pointers.values()],r=stage.getBoundingClientRect(),cx=(a.x+b.x)/2-r.left,cy=(a.y+b.y)/2-r.top,d=Math.hypot(a.x-b.x,a.y-b.y)||1;pinchStart={distance:d,scale,worldX:(cx-x)/scale,worldY:(cy-y)/scale};panStart=null}
});
stage.addEventListener('pointermove',e=>{
  ...
  if(pointers.size===2&&pinchStart){const[a,b]=[...pointers.values()],r=stage.getBoundingClientRect(),cx=(a.x+b.x)/2-r.left,cy=(a.y+b.y)/2-r.top,d=Math.hypot(a.x-b.x,a.y-b.y)||1;scale=clamp(pinchStart.scale*(d/pinchStart.distance),minScale*.75,maxScale);x=cx-pinchStart.worldX*scale;y=cy-pinchStart.worldY*scale;applyTransform()}
});
function release(e){pointers.delete(e.pointerId);if(!pointers.size){panStart=pinchStart=null;stage.classList.remove('dragging')}else if(pointers.size===1){const[r]=[...pointers.values()];panStart={pointerX:r.x,pointerY:r.y,x,y};pinchStart=null}}
stage.addEventListener('pointerup',release);stage.addEventListener('pointercancel',release);
window.addEventListener('resize',fitMap,{passive:true});document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!keyboard.hidden)closeKeyboard()});
window.ShaelvienPrototype=Object.freeze({mapTruth:MAP_TRUTH,getViewerState:()=>({activeStratum,viewerZ,...splitZ(viewerZ),parallaxOverride,parallax:lastMix,focusPath:focusPath.map(v=>({...v})),focusSelected,keyboardOpen:!keyboard.hidden,keyboardMode,toolMode})});
shortcut.value='surface';renderKeyboardTabs();renderState();
})();
