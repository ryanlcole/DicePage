...
function regionEditableAssetRows(){
  const regionId=activeRegionMapId();
  return userLayers
...
function closeRecursiveAssetList(){
...
function renderRecursiveAssetList(){
  ...
  for(const item of rows){
    ...
    const visible=document.createElement('button');visible.type='button';visible.className='recursive-icon';visible.textContent=recursive?.visible===false?'○':'◉';visible.setAttribute('aria-pressed',String(recursive?.visible!==false));visible.setAttribute('aria-label',recursive?.visible===false?`Show ${item.name||'asset'}`:`Hide ${item.name||'asset'}`);visible.addEventListener('click',()=>updateRegionAssetFromList(item,'visible'));
...
function setSelectedStackPin(pin){
  if(!selectedImage)return false;
  pin=['front','back'].includes(pin)?pin:'';
  const members=linkedSelectionMembers(selectedImage).filter(member=>!member.sourceLocked);
  if(!members.length)return false;
  const next=members.every(member=>member.stackPin===pin)?'':pin;
  ...
  announce(next==='front'
    ...
    :next==='back'
      ?'Selected asset pinned visually behind other editable assets. Tier and layer identity were not changed.'
      :'Visual stack pin cleared. Normal tier, layer, and stack order restored.');
  return true;
}
...
function appendAssetInteractionControls(item=selectedImage){
  ...
  keyboardKeys.append(
    ...
    toolKey(item.stackPin==='front'?'FRONT ✓':'FRONT',item.stackPin==='front'?'pinned above other assets':'pin visually above other assets',()=>setSelectedStackPin('front')),
    toolKey(item.stackPin==='back'?'BACK ✓':'BACK',item.stackPin==='back'?'pinned behind other assets':'pin visually behind other assets',()=>setSelectedStackPin('back')),
...
function renderKeyboardKeysContent(){
  ...
  if(keyboardMode==='Layers'){
    ...
    keyboardMode='Viewer';closeRecursiveAssetList();
  }
  if(recursiveAssetListPanel&&!recursiveAssetListPanel.hidden&&keyboardMode!=='Layers')closeRecursiveAssetList();
