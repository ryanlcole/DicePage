(()=>{
 ...
 let gameIndex=0,scanQueued=false,gameCssPromise=null;
 ...
 function ensureGameCss(){
  ...
  gameCssPromise=new Promise((resolve,reject)=>{
   ...
   if(existing){if(existing.sheet)resolve();else existing.addEventListener('load',resolve,{once:true});return;}
 ...
 function decorateTokenButtons(){
  ...
  if(pawn){
   pawn.parentElement?.classList.add('character-token-source');pawn.classList.add('character-front-token');pawn.textContent='';
   const src=tokenSource()?.dataset.frontToken||'';pawn.classList.toggle('has-character-token',!!src);pawn.classList.toggle('no-character-token',!src);
   ...
   const name=tokenSource()?.dataset.character||'selected character';pawn.parentElement?.setAttribute('aria-label',`Add ${name||'selected character'} front token to pallet`);pawn.parentElement?.setAttribute('title',name||'Character token');
  }
 ...
 function gameChoice(index){const b=document.createElement('button');b.type='button';b.className='calforth-game-choice'+(index===gameIndex?' active':'');b.dataset.index=index;const thumb=document.createElement('span');thumb.className=`calforth-game-thumb atlas-${index}`;const label=document.createElement('span');label.textContent=gameNames[index];b.append(thumb,label);b.addEventListener('click',()=>selectGame(index));return b;}
 function selectGame(index){gameIndex=index;const overlay=document.querySelector('.calforth-game-overlay');if(!overlay)return;overlay.querySelectorAll('.calforth-game-choice').forEach((b,i)=>b.classList.toggle('active',i===index));const stage=overlay.querySelector('.calforth-game-stage');if(stage)stage.className=`calforth-game-stage atlas-${index}`;const title=overlay.querySelector('.calforth-selected-game');if(title)title.textContent=gameNames[index];}
 function closeGames(){document.querySelector('.calforth-game-overlay')?.remove()}
 async function openGames(){
  closeGames();
  try{await ensureGameCss();}catch(err){console.error('Calforth game art failed to load',err);return;}
 ...
 function queueScan(){if(scanQueued)return;scanQueued=true;requestAnimationFrame(()=>{scanQueued=false;decorateTokenButtons()});}
 const root=document.querySelector('#app')||document.body;const mo=new MutationObserver(queueScan);mo.observe(root,{subtree:true,childList:true,attributes:true,attributeFilter:['data-front-token','data-character']});
 addEventListener('rist:app-ready',queueScan);if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',queueScan,{once:true});else queueScan();setTimeout(queueScan,1200);
})();
