(()=>{
 ...
 function updateToggle(){const b=q('.rist-rp-mode-button');if(b)b.textContent=mode;}
 ...
 function buildModeDropdown(){
  ...
  function openMenu(){
   ...
   for(const [code,label,range] of MODES){
    ...
    option.addEventListener('click',()=>{mode=code;button.textContent=mode;const input=q('.rist-chat-line-input');if(input)setLiveValue(input.value);closeMenu();});menu.appendChild(option);
   }
 ...
 function enhance(){
  ...
  send.addEventListener('click',()=>{const entry=modeEntry();lastSent={code:entry[0],verb:entry[1],line:input.value.trim()};setLiveValue(input.value);requestAnimationFrame(()=>{q('.home-chat-send')?.click();setTimeout(()=>{input.value='';setLiveValue('');formatVisibleOutputs();},0);});});
  ...
  actions.append(send,log,settings);wrap.append(modeDrop,input,actions);setLiveValue(input.value);formatVisibleOutputs();
 }
 function applyExternal(text){const parsed=parseTyped(text);const input=q('.rist-chat-line-input');if(input){input.value=parsed.line;setLiveValue(parsed.line);}return {code:mode,verb:modeEntry()[1],line:parsed.line};}
 ...
 window.RistRoleplayCommands={apply:applyExternal,format:formatLine,get code(){return mode},get verb(){return modeEntry()[1]},modes:MODES.map(m=>({code:m[0],label:m[1],range:m[2]}))};
