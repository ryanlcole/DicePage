(()=>{
 ...
 function ensureSelector(){
  ...
  b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();renderCast();});
 ...
 function hideLegacyRegions(){
  const shell=q('.rist.release-world>.release-world-shell');
  ['.release-public-region','.release-private-region','.release-footer-region'].forEach(sel=>{const el=q(`:scope>${sel}`,shell);if(el)el.style.setProperty('display','none','important')});
 }
 ...
 function cardsNode(cards){
  const wrap=document.createElement('div');wrap.className='rist-cast-cards';
  if(!cards.length){const empty=document.createElement('button');empty.type='button';empty.className='rist-cast-empty';empty.textContent='+ Add current character card';empty.addEventListener('click',addCurrentCharacter);wrap.appendChild(empty);return wrap;}
  cards.forEach(c=>wrap.appendChild(cardNode(c)));return wrap;
 }
 ...
 function propagate(cards,source){
  for(const child of cards.filter(c=>c.linkedTo===source.id)){child.command=source.command;propagate(cards,child)}
 }
 function controlsNode(cards){
  ...
  if(!card){wrap.textContent='Add a character card to begin.';return wrap;}
 ...
 function renderCast(){
  ...
  root.classList.remove('deck-chat','deck-dice','deck-assets','deck-logs','deck-custom-editor');root.classList.add('deck-cast');hideLegacyRegions();
  ...
  const [r1,r2,r3]=deckRows();setRow(r1,headerNode(cards));setRow(r2,cardsNode(cards));setRow(r3,controlsNode(cards));
 }
 function patch(){ensureSelector();if(q('.rist.release-world')?.classList.contains('deck-cast')){const active=q('.rist-section-list>button.active');if(active?.hasAttribute('data-cast-section'))return;}}
