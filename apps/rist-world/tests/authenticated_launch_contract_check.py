...
def main() -> None:
    ...
    perceiver_spatial = (ROOT.parents[1] / "site" / "relic-home" / "perceiver" / "perceiver-spatial.js").read_text(encoding="utf-8")
    ...
    require(perceiver, "FOCUS · AUTO", "Perceiver must expose automatic/manual focus control")
    require(perceiver, "CALIBRATE TILT CENTER", "Perceiver must expose explicit tilt calibration")
    require(perceiver, "perceiver-spatial.js", "Perceiver must load the spatial controller")
    require(perceiver_spatial, "videoRect()", "manual focus must map taps to the visible video rectangle")
    require(perceiver_spatial, "if(x<vr.left||x>vr.left+vr.width||y<vr.top||y>vr.top+vr.height)return", "manual focus must ignore black-bar taps")
    require(perceiver_spatial, "DeviceOrientationEvent", "tilt must use device orientation when supported")
    require(perceiver_spatial, "Orientation.requestPermission()", "iOS tilt permission must be requested explicitly")
    require(perceiver_spatial, "this.tiltButton.addEventListener", "tilt permission must originate from the Tilt control")
    require(perceiver, "const tilt=spatial?.offsets?.()", "movie motion and tilt must blend in the Perceiver renderer")
    require(perceiver, "reference=spatial?.focusSpeed?.()??1", "selected focus plane must become the zero-motion reference")
