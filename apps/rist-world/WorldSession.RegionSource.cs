...
public sealed partial class WorldSession
{
    public async Task<AwsAuthorityClient.WorldSource?> LoadRegionSourceAsync(string regionId)
    {
        ...
        if (privateOwner)
        {
            ...
            if (parent?.State is not { ValueKind: JsonValueKind.Object } parentState)
                throw new InvalidOperationException("Publish the private WorldBuilder map before opening a region.");
            ...
            return new AwsAuthorityClient.WorldSource(worldId, projected, parent.UpdatedAtUtc);
        }
        ...
        return authority is null
            ? null
            : await authority.GetRegionSourceAsync(worldId, region.RegionId);
    }
        ...
        JsonElement userLayers)
    {
        if (!HasActiveWorld || !IsLoggedIn)
            throw new InvalidOperationException("Choose a world before saving region changes.");
        ...
        if (privateOwner)
        {
            var parent = await LoadWorldBuilderSourceAsync();
            if (parent?.State is not { ValueKind: JsonValueKind.Object } parentState)
                throw new InvalidOperationException("Publish the private WorldBuilder source before editing a region.");
            ...
            return await SaveWorldBuilderSourceAsync(merged);
        }
