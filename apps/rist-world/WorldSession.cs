...
public sealed partial class WorldSession(HttpClient http, IJSRuntime js, DiscordAuthClient auth)
{
    ...
    public void StageTile(AtlasTile tile){var key=$"tile:{tile.Id}";if(StagedAssets.All(x=>x.Key!=key))StagedAssets.Add(new(key,"tile",tile.Name,tile.Image,tile.SourceWidth,tile.SourceHeight,tile.CropX,tile.CropY,tile.CropWidth,tile.CropHeight,Role=="GM"?HandCard.Approved:HandCard.Pending,tile.AssetKind,tile.AuthoredDepth,tile.DefaultTierIndex,tile.DefaultLayerOffset,tile.DefaultFootprint,tile.FrameCount,tile.FramesPerSecond));Notify();}
    ...
    public void PlaceStaged(StagedAsset staged,double x,double y,double placementZoom){if(Role=="PC"&&staged.ApprovalStatus!=HandCard.Approved)return;x=Math.Clamp(x,0,1);y=Math.Clamp(y,0,1);if(staged.Kind=="tile"){var footprint=staged.DefaultFootprint>0?staged.DefaultFootprint:1.0/Math.Max(placementZoom,.01);var tile=new TileItem(staged.Key[5..],staged.Name,staged.Image,x,y,staged.SourceWidth,staged.SourceHeight,staged.CropX,staged.CropY,staged.CropWidth,staged.CropHeight,1.0/Math.Max(footprint,.01)){CubeX=CubeX,CubeY=CubeY,CubeZ=CubeZ,PlaneIndex=PlaneIndex,TierIndex=staged.AuthoredDepth?staged.DefaultTierIndex:TierIndex,LayerOffset=staged.AuthoredDepth?staged.DefaultLayerOffset:LayerOffset,AssetKind=staged.AssetKind,AuthoredDepth=staged.AuthoredDepth,FrameCount=Math.Max(1,staged.FrameCount),FramesPerSecond=Math.Max(0,staged.FramesPerSecond)};PlacedTiles.Add(tile);}else Pieces.Add(new(staged.Kind,x,y,staged.Kind=="pin"?Math.Max(placementZoom,.01):1));Notify();}
