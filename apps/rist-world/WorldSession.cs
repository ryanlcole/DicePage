...
public sealed partial class WorldSession(HttpClient http, IJSRuntime js)
{
    ...
    public void PlacePin(double x,double y,double placementZoom){x=Math.Clamp(x,0,1);y=Math.Clamp(y,0,1);Pieces.Add(new("pin",x,y,Math.Max(placementZoom,.01)));Notify();}
