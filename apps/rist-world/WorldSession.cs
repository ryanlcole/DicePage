...
public sealed partial class WorldSession(HttpClient http, IJSRuntime js)
{
    ...
    private static int FinalFrameForValue(DiceSpec die,int value){var magnitude=Math.Abs(value);var frameValue=die.ValueOffset==0?magnitude:magnitude-1;return Math.Clamp(frameValue*2,0,die.FrameCount-1);}
    public async Task RollAsync(string key,int? selectedMagnitude){var die=Dice(key);if(die is null)return;var (x,y)=OpenRollPosition();var item=new RollItem(die.Key,die.Label,0,Random.Shared.Next(die.FrameCount),x,y);Rolls.Add(item);Notify();var steps=Random.Shared.Next(13,23);for(var n=0;n<steps;n++){var i=Rolls.IndexOf(item);if(i<0)return;item=item with{Frame=(item.Frame+1)%die.FrameCount};Rolls[i]=item;Notify();await Task.Delay(52+Math.Min(n*3,34));}var magnitude=selectedMagnitude.HasValue&&(die.Key is "d5-bonus" or "d5-penalty")?Math.Clamp(selectedMagnitude.Value,1,5):(die.ValueOffset==0?Random.Shared.Next(die.Sides):Random.Shared.Next(1,die.Sides+1));var value=magnitude*die.Sign;var finalFrame=FinalFrameForValue(die,value);var finalIndex=Rolls.IndexOf(item);if(finalIndex>=0)Rolls[finalIndex]=item with{Value=value,Frame=finalFrame};Notify();}
