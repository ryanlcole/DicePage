...
# 050596.python.scp.line59.comment still a bit confused about log(n, ...) vs logger.info/debug()


# 050597.python.scp.line62.comment Returns distinguished name of SCP.
def ScpCreate(
    ...
    if not dns_name:
        # 050600.python.scp.line76.comment Get the DNS name of the local computer
        dns_name = win32api.GetComputerNameEx(win32con.ComputerNameDnsFullyQualified)
    # 050601.python.scp.line78.comment Get the distinguished name of the computer object for the local computer
    if dn is None:
        dn = win32api.GetComputerObjectName(win32con.NameFullyQualifiedDN)

    # 050602.python.scp.line82.comment Compose the ADSpath and bind to the computer object for the local computer
    comp = adsi.ADsGetObject("LDAP://" + dn, adsi.IID_IDirectoryObject)

    # 050603.python.scp.line85.comment Publish the SCP as a child of the computer object
    keywords = keywords or []
    # 050604.python.scp.line87.comment Fill in the attribute values to be stored in the SCP.
    attrs = [
    ...
    # 050605.python.scp.line114.comment Wrap in a usable IDispatch object.
    new = Dispatch(new)
    # 050606.python.scp.line116.comment And allow access to the SCP for the specified account name
...
def ScpDelete(container_name, dn=None):
    ...
    # 050607.python.scp.line126.comment Compose the ADSpath and bind to the computer object for the local computer
    comp = adsi.ADsGetObject("LDAP://" + dn, adsi.IID_IDirectoryObject)
    comp.DeleteDSObject("cn=" + container_name)
    logger.info("Deleted service connection point '%s'", container_name)


# 050608.python.scp.line132.comment This function is described in detail in the MSDN article titled
# 050609.python.scp.line133.comment "Enabling Service Account to Access SCP Properties"
# 050610.python.scp.line134.comment From that article:
# 050611.python.scp.line135.comment The following sample code sets a pair of ACEs on a service connection point
# 050612.python.scp.line136.comment (SCP) object. The ACEs grant read/write access to the user or computer account
# 050613.python.scp.line137.comment under which the service instance will be running. Your service installation
# 050614.python.scp.line138.comment program calls this code to ensure that the service will be allowed to update
# 050615.python.scp.line139.comment its properties at run time. If you don't set ACEs like these, your service
# 050616.python.scp.line140.comment will get access-denied errors if it tries to modify the SCP's properties.
# 050617.python.scp.line141.comment
# 050618.python.scp.line142.comment The code uses the IADsSecurityDescriptor, IADsAccessControlList, and
# 050619.python.scp.line143.comment IADsAccessControlEntry interfaces to do the following:
# 050620.python.scp.line144.comment * Get the SCP object's security descriptor.
# 050621.python.scp.line145.comment * Set ACEs in the DACL of the security descriptor.
# 050622.python.scp.line146.comment * Set the security descriptor back on the SCP object.
...
def AllowAccessToScpProperties(
    ...
    # 050628.python.scp.line157.comment If no service account is specified, service runs under LocalSystem.
    # 050629.python.scp.line158.comment So allow access to the computer account of the service's host.
    if accountSAM:
        trustee = accountSAM
    else:
        # 050630.python.scp.line162.comment Get the SAM account name of the computer object for the server.
        trustee = win32api.GetComputerObjectName(win32con.NameSamCompatible)

    # 050631.python.scp.line165.comment Get the nTSecurityDescriptor attribute
    ...
    for sguid in schemaIDGUIDs:
        ...
        # 050632.python.scp.line173.comment Set the properties of the ACE.
        # 050633.python.scp.line174.comment Allow read and write access to the property.
        ace.AccessMask = ADS_RIGHT_DS_READ_PROP | ADS_RIGHT_DS_WRITE_PROP

        # 050634.python.scp.line177.comment Set the trustee, which is either the service account or the
        # 050635.python.scp.line178.comment host computer account.
        ace.Trustee = trustee

        # 050636.python.scp.line181.comment Set the ACE type.
        ace.AceType = ADS_ACETYPE_ACCESS_ALLOWED_OBJECT

        # 050637.python.scp.line184.comment Set AceFlags to zero because ACE is not inheritable.
        ace.AceFlags = 0

        # 050638.python.scp.line187.comment Set Flags to indicate an ACE that protects a specified object.
        ace.Flags = ADS_FLAG_OBJECT_TYPE_PRESENT

        # 050639.python.scp.line190.comment Set ObjectType to the schemaIDGUID of the attribute.
        ace.ObjectType = sguid

        # 050640.python.scp.line193.comment Add the ACEs to the DACL.
        acl.AddAce(ace)

    # 050641.python.scp.line196.comment Write the modified DACL back to the security descriptor.
    sd.DiscretionaryAcl = acl
    # 050642.python.scp.line198.comment Write the ntSecurityDescriptor property to the property cache.
    setattr(scpObject, attribute, sd)
    # 050643.python.scp.line200.comment SetInfo updates the SCP object in the directory.
    scpObject.SetInfo()
    logger.info("Set security on object for account %r", trustee)


# 050644.python.scp.line205.comment Service Principal Names functions from the same sample.
# 050645.python.scp.line206.comment The example calls the DsWriteAccountSpn function, which stores the SPNs in
# 050646.python.scp.line207.comment Microsoft Active Directory under the servicePrincipalName attribute of the
# 050647.python.scp.line208.comment account object specified by the serviceAcctDN parameter. The account object
# 050648.python.scp.line209.comment corresponds to the logon account specified in the CreateService call for this
# 050649.python.scp.line210.comment service instance. If the logon account is a domain user account,
# 050650.python.scp.line211.comment serviceAcctDN must be the distinguished name of the account object in
# 050651.python.scp.line212.comment Active Directory for that user account. If the service's logon account is the
# 050652.python.scp.line213.comment LocalSystem account, serviceAcctDN must be the distinguished name of the
# 050653.python.scp.line214.comment computer account object for the host computer on which the service is
# 050654.python.scp.line215.comment installed. win32api.TranslateNames and win32security.DsCrackNames can
# 050655.python.scp.line216.comment be used to convert a domain\account format name to a distinguished name.
def SpnRegister(
    ...
    # 050659.python.scp.line225.comment Bind to a domain controller.
    # 050660.python.scp.line226.comment Get the domain for the current user.
    samName = win32api.GetUserNameEx(win32api.NameSamCompatible)
    samName = samName.split("\\", 1)[0]

    if not serviceAcctDN:
        # 050661.python.scp.line231.comment Get the SAM account name of the computer object for the server.
    ...
    # 050662.python.scp.line235.comment Get the name of a domain controller in that domain.
    ...
    # 050663.python.scp.line242.comment Bind to the domain controller.
    handle = win32security.DsBind(info["DomainControllerName"])

    # 050664.python.scp.line245.comment Write the SPNs to the service account or computer account.
    ...
    # 050669.python.scp.line254.comment Unbind the DS in any case (but Python would do it anyway)
    handle.Close()


def UserChangePassword(username_dn, new_password):
    # 050670.python.scp.line259.comment set the password on the account.
    # 050671.python.scp.line260.comment Use the distinguished name to bind to the account object.
    accountPath = "LDAP://" + username_dn
    user = adsi.ADsGetObject(accountPath, adsi.IID_IADsUser)

    # 050672.python.scp.line264.comment Set the password on the account.
    user.SetPassword(new_password)


# 050673.python.scp.line268.comment functions related to the command-line interface
...
def do_SpnCreate(po):
    ...
    # 050674.python.scp.line320.comment The 'service name' is the dn of our scp.
    if g_createdSCP is None:
        # 050675.python.scp.line322.comment Could accept an arg to avoid this?
        _option_error(po, "ScpCreate must have been specified before SpnCreate")
    # 050676.python.scp.line324.comment Create a Service Principal Name"
...
def main():
    ...
    po = (parser, options)
    # 050677.python.scp.line466.comment fixup misc
    try:
        options.port = int(options.port)
    except (TypeError, ValueError):
        parser.error("--port must be numeric")
    # 050678.python.scp.line471.comment fixup log-level
    try:
    ...
    try:
        sl = logger.setLevel
        # 050679.python.scp.line481.comment logger is a real logger
    except AttributeError:
        # 050680.python.scp.line483.comment logger is logging module
        sl = logging.getLogger().setLevel
    sl(log_level)
    # 050681.python.scp.line486.comment Check -q/-v
    if options.quiet and options.verbose:
        parser.error("Can't specify --quiet and --verbose")
    if options.quiet:
        options.verbose -= 1
    verbose = options.verbose
    # 050682.python.scp.line492.comment --test
    if options.test:
        if args:
    ...
    # 050683.python.scp.line510.comment check args
    if not args:
    ...
    # 050684.python.scp.line517.comment Patch up account-name.
    ...
    # 050685.python.scp.line529.comment do it.
    for arg in args:
