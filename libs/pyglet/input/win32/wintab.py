...
class WintabTablet(Tablet):
    def __init__(self, index):
        ...
        # 030618.python.wintab.line65.comment phys_cursors = hardware & wintab.HWC_PHYSID_CURSORS
...
class WintabTabletCanvas(TabletCanvas):
    ...
    def __init__(self, device, window, msg_base=wintab.WT_DEFBASE):
        ...
        # 030619.python.wintab.line94.comment Get the extension masks available. Only need to do this once.
        global _extension_masks
        if not _extension_masks:
            _extension_masks = get_extension_masks()

        # 030620.python.wintab.line99.comment Just use system context, for similarity w/ os x and xinput.
        # 030621.python.wintab.line100.comment WTI_DEFCONTEXT detaches mouse from tablet, which is nice, but not
        # 030622.python.wintab.line101.comment possible on os x afiak.
        ...
        # 030623.python.wintab.line107.comment If you change this, change definition of PACKET also.
        ...
        # 030628.python.wintab.line134.comment Override test
    ...
    def _event_wt_packetext(self, msg, wParam, lParam):
        ...
        # 030630.python.wintab.line230.comment Proper context exists in the packet, not the lParam.
    ...
    def _event_wt_proximity(self, msg, wParam, lParam):
        ...
        if not lParam & 0xffff0000:
            # 030631.python.wintab.line248.comment Not a hardware proximity event
            return

        if not lParam & 0xffff:
            # 030632.python.wintab.line252.comment Going out
            self.dispatch_event('on_leave', self._current_cursor)

        # 030633.python.wintab.line255.comment If going in, proximity event will be generated by next event, which
        # 030634.python.wintab.line256.comment can actually grab a cursor id.
        self._current_cursor = None

    # 030635.python.wintab.line259.comment Events
...
class WintabTabletCursor:
    def __init__(self, device, index):
        ...
        # 030636.python.wintab.line301.comment A whole bunch of cursors are reported by the driver, but most of
        # 030637.python.wintab.line302.comment them are hogwash.  Make sure a cursor has at least X and Y data
        # 030638.python.wintab.line303.comment before adding it to the device.
...
def get_tablets(display=None):
    # 030639.python.wintab.line393.comment Require spec version 1.1 or greater
