# 048434.python.test_win32gui.line1.comment tests for win32gui
import array
import operator
import sys
import unittest
...
class TestPyGetString(unittest.TestCase):
    def test_get_string(self):
        # 048435.python.test_win32gui.line14.comment test invalid addresses cause a ValueError rather than crash!
...
class TestPyGetMemory(unittest.TestCase):
    def test_ob(self):
        # 048436.python.test_win32gui.line22.comment Check the PyGetMemory result and a bytes string can be compared
    ...
    def test_memory_index(self):
        # 048437.python.test_win32gui.line31.comment Check we can index into the buffer object returned by PyGetMemory
        test_data = b"\0\1\2\3\4\5\6"
        c = array.array("b", test_data)
        addr, buflen = c.buffer_info()
        got = win32gui.PyGetMemory(addr, buflen)
    ...
    def test_memory_slice(self):
        # 048438.python.test_win32gui.line39.comment Check we can slice the buffer object returned by PyGetMemory
        test_data = b"\0\1\2\3\4\5\6"
        c = array.array("b", test_data)
        addr, buflen = c.buffer_info()
        got = win32gui.PyGetMemory(addr, buflen)
    ...
    def test_real_view(self):
        # 048439.python.test_win32gui.line47.comment Do the PyGetMemory, then change the original memory, then ensure
        # 048440.python.test_win32gui.line48.comment the initial object we fetched sees the new value.
        test_data = b"\0\1\2\3\4\5\6"
        c = array.array("b", test_data)
        addr, buflen = c.buffer_info()
        got = win32gui.PyGetMemory(addr, buflen)
    ...
    def test_memory_not_writable(self):
        # 048441.python.test_win32gui.line58.comment Check the buffer object fetched by PyGetMemory isn't writable.
        test_data = b"\0\1\2\3\4\5\6"
        c = array.array("b", test_data)
        addr, buflen = c.buffer_info()
        got = win32gui.PyGetMemory(addr, buflen)
